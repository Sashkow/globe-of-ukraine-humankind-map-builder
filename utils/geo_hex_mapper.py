...
class GeoHexMapper:
    ...
    def _calculate_hex_grid(self):
        ...
        # - Horizontal spacing (column to column): 0.75 * hex_width = 0.75 * (2 * hex_size)
        # - Vertical spacing (row to row): hex_height = hex_size * sqrt(3)
        #
        # Grid width in meters: (width - 1) * 0.75 * hex_width + hex_width
        #                      = width * hex_width - 0.25 * hex_width
        #                      = (width - 0.25) * 2 * hex_size
        # Grid height in meters: height * hex_height = height * hex_size * sqrt(3)

        # Solve for hex_size based on width constraint
        hex_size_from_width = width_m / ((self.width - 0.25) * 2)

        # Solve for hex_size based on height constraint
        hex_size_from_height = height_m / (self.height * np.sqrt(3))
